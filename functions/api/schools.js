export async function onRequest(context) {
  ...
  try {
    if (method === 'GET') {
      let list = null;
      if (isDbBound) {
        const raw = await kv.get('schools_data');
    ...
    if (method === 'POST') {
      ...
      if (isDbBound) {
        await kv.put('schools_data', JSON.stringify(body.data));
        return new Response(JSON.stringify({ success: true }), { headers, status: 200 });
      } else {
        return new Response(JSON.stringify({ success: false, error: 'Database not bound (SCHOOLS_KV or ELIMU_DB)' }), { headers, status: 400 });
      }
    }
